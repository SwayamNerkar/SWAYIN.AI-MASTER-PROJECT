...
class PredictionService(PredictionServiceInterface):
    ...
    async def predict_latest(self, symbol: str, timeframe: str = "1m") -> PredictionResponse:
        """
        Generates inference prediction for current market state of symbol.
        """
        sym = symbol.upper()
        ...
        # Load metadata if exists, else fallback to training run or on-demand artifact
        if not os.path.exists(model_path) or not os.path.exists(scaler_path):
            # Auto-trigger training if model not found for development test loop
        ...
        # Get latest market candles & calculate features & regime
        ...
        # Construct single-row inference feature dataframe
